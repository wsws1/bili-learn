...
1. 启动器里打开「允许局域网访问」，先试它自动列出的链接；
2. 如果打不开，说明容器 IP ≠ 宿主 IP，按下面任一种方式拿**鸿蒙系统在 WiFi 下的真实 IP**：
   - 鸿蒙：设置 → WLAN → 当前连接的 WiFi → 详情（或长按 → 修改网络 → 高级）→ 查看 IP 地址；
   - 路由器后台：已连接设备列表里找到这台手机；
   - 另一台同 WiFi 设备：用 Fing 等工具扫描局域网。
3. 把拿到的 IP 填进启动器「输入鸿蒙系统 WiFi IP」输入框点「检测」；
   显示「可达：http://<IP>:3210，服务正常」后，鸿蒙浏览器或电脑访问该地址即可；
4. 若检测不可达：检查卓易通的网络模式（共享宿主机网络 / 独立虚拟网卡）、
   路由器是否开启 AP 隔离（设备互相隔离），或手机是否开了热点隔离。

### 卓易通 NAT（容器独立网段）的访问方案

卓易通通常给容器分配独立网段（IP 前缀与鸿蒙宿主机不同），此时外部设备无法直接
访问容器内监听的端口，需要做「映射」或「共享网络」：

1. 首选：卓易通设置里找「共享宿主机网络 / 网络共享」——开启后容器与宿主机共用 IP，
   `http://鸿蒙WiFiIP:3210` 即可直接访问；
2. 若提供端口映射：把宿主机的某个端口（如 3210）转发到容器 IP 的 3210；
3. 通用兜底：在容器内安装 Tailscale / ZeroTier 并登录，把 3210 加入虚拟网，
   任意已登录设备通过虚拟 IP 访问；
4. 仅同机使用：容器自己的浏览器访问 `http://127.0.0.1:3210` 不受 NAT 影响。
